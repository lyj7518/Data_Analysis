...
# tidyr 패키지의 gather() 사용 -> key : 새로 만들 컬럼
covid_kor2 <- gather(covid_kor, key="type", value="count", Confirmed:Current)
str(covid_kor2)

covid_kor2$type <- factor(covid_kor2$type, levels=c("Confirmed", "Current", "Recovered", "Deaths"), labels=c("확진", "치료중","완치","사망")) ## 타입을 text -> factor로 변환, levels = 원래 이름, labels = 출력될 이름
...
# aggregate() 함수 사용 -> x:합칠 데이터, by:기준
...
# dplyr 패키지의 arrange() 함수 사용 ->확진자수 기준으로 내림차순(-)
